...
// End Debug.cc information    ----------------------------------------------------------
    ...
    // An anonymous namespace is used to make identifiers local to the translation unit 
    // (typically a .cpp file). This is useful for defining variables or functions that 
...
void setup() {
  ...
  while (!Serial){} 
    ...
    spi.frequency(60000000);             // Set up your frequency.
    spi.format(8, 0); 
    // List files and directories
    // Serial.println("Listing files:");
    // list_files_in_directory("/fs/Model");
    
    // Read model
    
    WriteFileInChunks(MODEL_PATH);
    
    Serial.println("Done Writing ");
    Serial.println(model_data.address);
    Serial.println(model_data.size);

    uint8_t* model_tflite = (uint8_t*)malloc(model_data.size * sizeof(uint8_t));  
    SpiReadByteArray(model_data.address, model_data.size, model_tflite);
...
void loop() {
   myCounter +=1;
    ...
    Serial.println( String(y_val) + ",0," +String(predicted) );
